...
// Helper: structured log for participant entries
function logParticipantEntry(user, action, details = {}) {
  try {
    ...
    console.log("GIVEAWAY_ENTRY", JSON.stringify(entry));
  } catch (err) {
    // Don't let logging break the main flow
    console.log("GIVEAWAY_ENTRY", user, action, details);
  }
}
...
async function connectToTikTok(username) {
    ...
    if (giveawayActive && giveawayConfig.enabled.commands) {
      if (
        ...
        if (!participants.includes(user)) {
          ...
          logParticipantEntry(user, "command", { message: chatData.comment });
  ...
  connection.on("gift", (giftData) => {
    ...
    if (giveawayActive && giveawayConfig.enabled.gifts) {
      if (giveawayConfig.gifts.includes(giftName)) {
        if (!participants.includes(user)) {
          participants.push(user);
          logParticipantEntry(user, "gift", { giftName });
          io.emit("participant-joined", user);
        }
      }
    }
  ...
  connection.on("like", (likeData) => {
    ...
    if (
      ...
      participants.push(user);
      logParticipantEntry(user, "like", { likeCount: userLikes[user] });
      io.emit("participant-joined", user);
    }
  });
...
io.on("connection", (socket) => {
  ...
  socket.on("draw-winner", () => {
    ...
    io.emit("giveaway-winner", { username: winner, index: winnerIndex });
  });
